...
const IMPL_STEPS = [
  { icon: "🔍", title: "Validasi Data Wilayah",
    desc: "Verifikasi data simulasi dengan data operasional dari dinas terkait sebelum dijadikan basis keputusan resmi." },
...
export default function PolicyBriefPage() {
  ...
  const initRegion = searchParams.get("region") ?? mockRegions[0].id;
  const [selectedId, setSelectedId] = useState(initRegion);
  const [copied, setCopied] = useState(false);

  const raw = getRegionById(selectedId) ?? mockRegions[0];

  // ── All data computed from raw region ──────────────────────────────────────
  const scored   = useMemo(() => scoreRegion(raw, "general"), [raw]);
  const brief    = useMemo(() => generatePolicyBrief(raw, "general"), [raw]);
  const insight  = useMemo(() => generateRegionInsight(raw, "general"), [raw]);
  const citizen  = useMemo(() => generateCitizenSummary(raw), [raw]);
  const explain  = useMemo(() => explainPriority(raw, "general"), [raw]);
  const top4     = useMemo(() => getDominantIndicators(raw.indicators, "general", 4), [raw]);
  ...
  async function handleCopy() {
    const text = [
      ...
      `Dokumen: ${docId}  |  Dibuat: ${dateStr}  |  Generated by CivicSense AI`,
      `Priority Score: ${scored.computedScore}/100 (${scored.computedCategory})`,
      "",
      "EXECUTIVE SUMMARY",
      brief.executiveSummary,
      "",
      "ANALISIS PRIORITAS (CivicSense AI)",
      insight.body,
      "",
      "SINYAL DATA UTAMA",
      ...
      <div className="space-y-5 pb-12">
        ...
        >
          <div>
            ...
            <p className="text-sm text-civic-muted">
              Dokumen ringkasan kebijakan berbasis data wilayah — dihasilkan otomatis oleh CivicSense AI.
            </p>
          </div>
          <Link
            to="/dashboard"
        ...
        >
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            ...
            <div className="shrink-0 rounded-2xl border border-white/20 bg-white/10 p-6 text-center backdrop-blur-sm min-w-[160px]">
              ...
              <p className="mt-2 text-xs text-white/40">Mode: General Priority</p>
            </div>
          </div>
        </header>
        ...
        <Section id="sec-priority" num="02" title="Priority Analysis" aiTagged>
          ...
          <div className={classNames(
            ...
            <span className="font-bold">💡 AI Rekomendasi: </span>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs leading-relaxed text-amber-700">
                Policy brief ini dihasilkan oleh simulasi CivicSense AI berdasarkan data dummy prototype.
                Pada implementasi nyata, hasil rekomendasi perlu divalidasi oleh pemerintah daerah dan data resmi terkait.{" "}
                {AI_DISCLAIMER}
              </p>
            </div>
          </div>
